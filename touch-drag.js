...
class TouchDrag {
  ...
  constructor() {
    ...
    this.options.minHoldTime && (TouchUI.HOLD_TIME = options.minHoldTime);
  ...
  dragMoveHandler(e) {
    if (this.dragStartAt) { //if drag started
      ...
      let prevStyle = window.getComputedStyle(e.target, null);
      this.dragStartStyle = {
    ...
    let dragX = (this.options.axis.indexOf('x') !== -1) ? move.x : 0;
    let dragY = (this.options.axis.indexOf('y') !== -1) ? move.y : 0;
